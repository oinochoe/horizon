...
float circle(in vec2 _st,in float _radius,in float blurriness){
  vec2 dist=_st;
  return 1.-smoothstep(_radius-(_radius*blurriness),_radius+(_radius*blurriness),dot(dist,dist)*10.);
}
...
float snoise3(vec3 v)
{
  ...
  vec4 p=permute(permute(permute(
      ...
      void main(){
        ...
        float c=circle(circlePos,.15,2.)*2.5;
        
        float offx=v_uv.x+sin(v_uv.y+u_time*.1);
        float offy=v_uv.y-u_time*.1-cos(u_time*.001)*.01;
